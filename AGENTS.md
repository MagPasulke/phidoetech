...
- **Build:** Vite 5 (`vite.config.js`, `base: '/'` wegen Custom Domain)
- **UI:** React 18 (function components + hooks, keine Klassen)
...
src/
  ...
  components/
    ...
    Pillars.jsx + .css         Die zwei Schwerpunkte (Beratung / Hands-on) als Karten
...
- Struktur der Übersetzungen spiegelt die Sektionen (`nav`, `hero`, `pillars`, `contact`, `footer`).
- Aufzählungen/Listen als Arrays (`hero.modules`, `hero.stats`, `pillars.*.items`) und im JSX per `.map()` rendern.
